...
def blog_thumbnail_directory(instance, filename):
    return 'actividades/{0}/{1}'.format(instance.title, filename)
...
class Calendario(models.Model):
    ...
    def __str__(self):
        return self.title or f"Calendario {self.pk}"
...
class ViewCount(models.Model):
    post = models.ForeignKey(Calendario, related_name='blogpost_view_count', on_delete=models.CASCADE)
